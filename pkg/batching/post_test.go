...
func TestPOST(t *testing.T) {
	...
	t.Run("should fail on raw graphQL multiple queries", func(t *testing.T) {
		resp := doRequest(
			h,
			"POST",
			"/graphql",
	...
	t.Run("decode failure", func(t *testing.T) {
		...
		assert.JSONEq(t, `{"errors":[{"message":"json request body could not be decoded: invalid character 'o' in literal null (expecting 'u')"}],"data":null}`, resp.Body.String())
	})
	...
	t.Run("parse failure multiple queries", func(t *testing.T) {
		...
		assert.Contains(t, []any{http.StatusOK, http.StatusUnprocessableEntity}, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.JSONEq(t, `[{"data":{"name":"test"}},{"errors":[{"message":"Unexpected !","locations":[{"line":1,"column":1}],"extensions":{"code":"GRAPHQL_PARSE_FAILED"}}],"data":null}]`, resp.Body.String())
	})
	...
	t.Run("validation failure multiple queries", func(t *testing.T) {
		...
		assert.Equal(t, http.StatusUnprocessableEntity, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.JSONEq(t, `[{"errors":[{"message":"Cannot query field \"title\" on type \"Query\".","locations":[{"line":1,"column":3}],"extensions":{"code":"GRAPHQL_VALIDATION_FAILED"}}],"data":null},{"data":{"name":"test"}}]`, resp.Body.String())
	})
	...
	t.Run("invalid variable multiple queries", func(t *testing.T) {
		...
		assert.Equal(t, http.StatusUnprocessableEntity, resp.Code, resp.Body.String())
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.JSONEq(t, `[{"errors":[{"message":"cannot use bool as Int","path":["variable","id"],"extensions":{"code":"GRAPHQL_VALIDATION_FAILED"}}],"data":null},{"errors":[{"message":"cannot use bool as Int","path":["variable","id2"],"extensions":{"code":"GRAPHQL_VALIDATION_FAILED"}}],"data":null}]`, resp.Body.String())
	})
	...
	t.Run("execution failure", func(t *testing.T) {
		...
		assert.Equal(t, resp.Header().Get("Content-Type"), "application/json")
		assert.JSONEq(t, `[{"errors":[{"message":"mutations are not supported"}],"data":null},{"errors":[{"message":"mutations are not supported"}],"data":null}]`, resp.Body.String())
	})
}
